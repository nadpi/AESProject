...
namespace AESProject
{
    class AESClass
    {
        ...
        public static string[,] turnStringintoMatrix(string strng)
        {
            int idx = 0;
            string[,] newMatr = new string[4, 4];
            for (int i = 0; i < 4; i++)
            {
                for (int j = 0; j < 4; j++)
                {
                    newMatr[j, i] = strng.Substring(idx, 2);
                    idx += 2;
                }
            }
            return newMatr;
        }
        public static string[,] keyExpansion(string[,]keyMatrix)
        {
            ...
            int k, i, j, idx, constIdx = 0, indx = 0;
            string tmp, nextTmp;
        ...
        public static string[,] createStateArr(string [,]plainMatrix, string [,]initKeyMatrix)
        {
            int stateXORValue, idx = 0, i, j;
        ...
        public static string Encrypt(string plainText, string key)
        {
            ...
            key = key.ToUpper();
            keyMatrix = turnStringintoMatrix(key);
            initKeyMatrix = turnStringintoMatrix(key);

            // Key Expansion
            fullKeyMatrix = keyExpansion(keyMatrix);
            plainMatrix = turnStringintoMatrix(plainText);

            // State Array (initial key matrix XOR plaintext)
            stateArray = createStateArr(plainMatrix, initKeyMatrix);
            for (i = 0; i < 16; i++)
            {
                for (j = 0; j < 16; j++)
                {
        ...
        public static string Decrypt(string cipherText, string key)
        {
            ...
            string[] plainArr = new string[cipherText.Length];
            string[,] newFullKeyMatrix = new string[4, 44];
            string plain = "";
            int i, j, k, x, idx = 0, indx = 0, constIdx = 0, fullKeyCnt = 0;
            int mixColCnt = 0, indxx = 0;
            ...
            // Key Expansion
            fullKeyMatrix = keyExpansion(keyMatrix);
            cipherMatrix = turnStringintoMatrix(cipherText);
            for (i = 0; i < 16; i++)
            {
                for (j = 0; j < 16; j++)
                {
            ...
            for (i = 0; i < 4; i++)
            {
                for (j = 0; j < 4; j++)
                {
                    stateXORValue = (Convert.ToInt32(cipherMatrix[j, i], 16)) ^ (Convert.ToInt32(newFullKeyMatrix[j, indxx], 16));
                    stateArray[j, i] = stateXORValue.ToString("X");
            ...
            char[] plainChar = new char[cipherMatrix.Length];
            string PT;
            idx = 0;
            for (i = 0; i < 4; i++)
            {
                for (j = 0; j < 4; j++)
                {
                    plainArr[idx] = prevStateArray[j, i];
                    plain += plainArr[idx];
                    idx++;
                }
            }
